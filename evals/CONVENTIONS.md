...
| --- | --- | --- | --- | --- |
| Negative clause in a **skill** description (`04-skill-description-antitrigger`) | Documented **as a remedy** for a skill that triggers too often ("Add negative triggers" — *The Complete Guide to Building Skills for Claude*); the docs say "make the description more specific"; `skill-creator` favours "pushy" descriptions | Confusable pair: triggering 1/10 → 10/10 with crossed clauses (`two-skills-compete`). **Isolated skill: no effect** — true triggers 60/60 with and without the clause, false triggers on near misses naming the excluded topics 0/40 with and without (Haiku, 2026-09-23). So: a WARN on confusable pairs (check 33), a NOTICE elsewhere | 2026-09-23 | next model |
